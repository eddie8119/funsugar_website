<template>
  <div>
    <section
      id="services-section"
      class="relative overflow-hidden px-12 py-16 lg:px-32"
    >
      <!-- Text wrapper start -->
      <div class="flex flex-col gap-8 md:flex-row">
        <h1 class="mb-4 text-5xl font-semibold uppercase">我們的服務</h1>
        <p class="mb-8 text-stone-500 xl:w-1/2">
          我的專業能幫你做出有質地的影像成品。
          專門協助宅產業類型客戶提供影像製作服務。 7
          個工作天提供您成品，並調整符合到您滿意的狀態。<br />
          <br />
          My specialty can help you make textured finished products.
          Specializing in assisting home industry customers to provide video
          production services. 7 working days to deliver your finished product
          and adjust it to your satisfaction.
        </p>
      </div>
      <!--  -->
      <div class="flex flex-col md:flex-row md:gap-8">
        <div
          class="my-8 flex flex-row items-center justify-between md:w-1/3 md:flex-col md:items-start md:justify-start"
        >
          <img
            src="@/assets/images/services/lifestyle.png"
            alt="interior"
            class="-z-50 md:mb-4"
          />
          <div class="w-2/3 text-right md:w-full md:text-left">
            <a
              href="https://www.behance.net/gallery/158438305/40-"
              target="_blank"
            >
              <h1
                class="delay-80 text-2xl font-semibold uppercase transition duration-150 ease-in-out hover:text-yellow-500"
              >
                室內空間作品攝影
              </h1>
            </a>
            <p class="capitalize text-stone-500">
              室內設計事務所完工作品集紀錄
            </p>
            <p class="capitalize text-stone-500">
              Interior Design Office Completed Portfolio Record
            </p>
          </div>
        </div>
        <div
          class="my-8 flex flex-row items-center justify-between md:w-1/3 md:flex-col md:items-start"
        >
          <div class="w-2/3 text-left md:order-2 md:w-full">
            <a
              href="https://www.behance.net/gallery/131481845/_"
              target="_blank"
            >
              <h1
                class="delay-80 text-2xl font-semibold uppercase transition duration-150 ease-in-out hover:text-yellow-500"
              >
                建材公司影片
              </h1>
            </a>
            <p class="capitalize text-stone-500">室內設計記錄影片</p>
            <p class="capitalize text-stone-500">
              Interior Design Interview Videos And Room Tour
            </p>
          </div>
          <img
            src="@/assets/images/services/moments.png"
            alt="film"
            class="-z-50 md:order-1 md:mb-4"
          />
        </div>
        <div
          class="my-8 flex flex-row items-center justify-between md:w-1/3 md:flex-col md:items-start"
        >
          <div class="w-1/3">
            <img
              src="@/assets/images/services/home.png"
              alt="Moments"
              class="-z-50 md:mb-4"
            />
          </div>
          <div class="w-2/3 text-right md:w-full md:text-left">
            <a
              href="https://docs.google.com/forms/d/e/1FAIpQLSf9Gz3Tbm2qxpIxiQVAZRqNx3ZBaO2R3LiQam-Wpyt-xno_kA/viewform"
              target="_blank"
            >
              <h1
                class="delay-80 text-2xl font-semibold uppercase transition duration-150 ease-in-out hover:text-yellow-500"
              >
                屋主新居落成家族紀錄片
              </h1>
            </a>
            <p class="capitalize text-stone-500">屋主新屋紀錄片(限台北市)</p>
            <p class="capitalize text-stone-500">
              The homeowner's new house record commission (Taipei City)
            </p>
          </div>
        </div>
      </div>
      <!-- Circle start -->
      <div
        class="absolute top-0 -right-20 -z-20 mt-16 h-64 w-64 rounded-full bg-yellow-400"
      />
    </section>
    <section
      id="positive-energy"
      class="relative overflow-hidden px-12 pb-16 lg:px-32"
    >
      <div class="mt-10 flex flex-col gap-8 md:flex-row">
        <h1 class="mb-4 text-5xl font-semibold uppercase">更多居家正能量</h1>
        <p class="mb-8 text-stone-500 xl:w-1/2">
          室內攝影委託費，收到費用即捐款 1 成至"家園議題的慈善機構"。<br />
          委託者個人平時的捐款也可兌換成攝影服務張數。
          用各種形式，帶給居家的正能量。
          <br />
        </p>
      </div>
      <!--  -->
      <div class="flex flex-col md:flex-row md:gap-8">
        <div
          class="my-8 flex flex-row items-center justify-between md:w-1/3 md:flex-col md:items-start md:justify-start"
        >
          <img
            src="@/assets/images/services/home.png"
            alt="interior"
            class="-z-50 md:mb-4"
          />
          <div class="w-2/3 text-right md:w-full md:text-left">
            <a
              href="https://donate.ccf.org.tw/19/shopcart-step1/210"
              target="_blank"
            >
              <h1
                class="delay-80 text-2xl font-semibold uppercase transition duration-150 ease-in-out hover:text-yellow-500"
              >
                家扶扶幼捐款項目
              </h1>
            </a>
            <p class="capitalize text-stone-500">
              每次委託費 1 成捐款至<a
                href="https://donate.ccf.org.tw/19/shopcart-step1/210"
                >"家園議題的慈善機構"</a
              >，收到委託人的匯款當下即捐出，創造善流動
            </p>
          </div>
        </div>
        <div
          class="my-8 flex flex-row items-center justify-between md:w-1/3 md:flex-col md:items-start"
        >
          <div class="w-2/3 text-left md:order-2 md:w-full">
            <a href="https://www.ccf.org.tw/support/project" target="_blank">
              <h1
                class="delay-80 text-2xl font-semibold uppercase transition duration-150 ease-in-out hover:text-yellow-500"
              >
                家扶捐款兌換攝影
              </h1>
            </a>
            <p class="capitalize text-stone-500">
              委託方捐款家扶基金會，每1200元捐款可兌換 1 張室內攝影<br />
            </p>
          </div>
          <img
            src="@/assets/images/services/home.png"
            alt="film"
            class="-z-50 md:order-1 md:mb-4"
          />
        </div>
        <div
          class="my-8 flex flex-row items-center justify-between md:w-1/3 md:flex-col md:items-start"
        >
          <div class="w-1/3">
            <img
              src="@/assets/images/services/home.png"
              alt="Moments"
              class="-z-50 md:mb-4"
            />
          </div>
          <div class="w-2/3 text-right md:w-full md:text-left">
            <a href="https://donations.hondao.org.tw/" target="_blank">
              <h1
                class="delay-80 text-2xl font-semibold uppercase transition duration-150 ease-in-out hover:text-yellow-500"
              >
                弘道老人捐款兌換攝影
              </h1>
            </a>
            <p class="capitalize text-stone-500">
              委託方捐款弘道老人福利基金會，每1200元捐款可兌換 1 張室內攝影<br />
            </p>
          </div>
        </div>
      </div>
    </section>
  </div>
</template>

<script>
  export default {};
</script>

<style></style>
